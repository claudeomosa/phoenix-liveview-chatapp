defmodule ChatAppWeb.RoomLive do
  ...
  def handle_info(%{event: "presence_diff", payload: %{joins: joins, leaves: leaves}}, socket) do
    join_messages =
      ...
      |> Enum.map(fn username ->
        %{type: :system, uuid: UUID.uuid4(), text: "#{username} joined the chat"}
      end)
    ...
    leave_messages =
      ...
      |> Map.keys()
      |> Enum.map(fn username ->
        %{type: :system, uuid: UUID.uuid4(), text: "#{username} left the chart"}
      end)
    ...
    {:noreply,
     assign(socket, messages: join_messages ++ leave_messages, online_users: online_users)}
  end
  ...
  def display_message(%{uuid: uuid, text: text, username: username}) do
    ~E"""
    <div class="border-b border-gray-600 py-3 flex items-start mb-4 text-sm ml-4" id="<%= uuid%>">
        <div class="flex-1 overflow-hidden">
          <div>
            <span class="font-bold text-red-300 cursor-pointer hover:underline"><%= username%></span>
            <span class="font-bold text-gray-400 text-xs"><%= Time.utc_now.hour %>:<%= Time.utc_now.minute%></span>
          </div>
